...
public final class InteractiveGraphView extends JPanel implements AutoCloseable {
    ...
    private static final String STYLESHEET = """
            ...
            node { shape: rounded-box; size-mode: fit; padding: 12px, 9px;
                   ...
                   stroke-width: 1px; text-color: #202632; text-size: 13px;
            ...
            edge { fill-color: #7c8798; arrow-shape: arrow; arrow-size: 9px, 5px;
                   text-color: #586170; text-size: 10px; }
            """;
    ...
    public InteractiveGraphView() {
        ...
        tools.add(button("Open source", this::openSelected));
        tools.addSeparator();
        tools.add(status);
    ...
    public void model(GraphViewModel value) {
        ...
        collapsed.forEach(this::hideDescendants);
    ...
    private void hideDescendants(String rootId) {
        ...
        hidden.forEach(id -> { Node node = graph.getNode(id); if (node != null) node.setAttribute("ui.hide"); });
        graph.edges().filter(edge -> hidden.contains(edge.getNode0().getId())
                || hidden.contains(edge.getNode1().getId())).forEach(edge -> edge.setAttribute("ui.hide"));
    }
