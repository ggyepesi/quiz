...
- **Declared Java fields** via reflection (`ViewableAdapter.getField` / `getAllFields`),
- **A dynamic property map** on a `WikidataDynamicObject` / `DynamicViewable`
...
reflection-only `ViewableFieldPaths.collect(config)` (misses a WDO's map-held fields,
...
|---|---|---|
| enumerate | `ViewableFieldPaths.collect(config)` (reflection) vs `collectFromSample` (FieldSet) | 5 vs 2 callers |
| read/write | `FieldAccess.readField` / `writeField` | `instanceof DynamicFields` |
| render | `ViewablePanel` (615, 740) | `instanceof DynamicFields` |
| serialize | `ViewableJson` (312, 337, 381, 441) | `instanceof DynamicFields` |
...
| config editor | `ViewablePanelConfigEditor` (111/153/175/217) | dynamic vs reflection rows |
| search value | `ViewablePanelSearchAndSort.extractValue` (289) | `instanceof DynamicFields` |
...
1. **Enumeration** — make `FieldSet`-based enumeration the single path. `collectFromSample`
   ...
   backing. *(searchViewables already routed — the first step.)*
...
3. **Render / serialize / facet / web** — replace each `instanceof DynamicFields` with
   ...
   - **3a — pure reads** *(DONE — a06a6dc)*: `ViewablePanelSearchAndSort.extractValue`,
     `FacetKeys.readField`, `ViewableJson.rawFieldValue`/`stringValueSingle`. Mechanical,
     behavior-preserving (a Viewable reads via `FieldSet.of(q).read`, non-Viewable nested
     owners keep reflection).
   - **3b — enumeration / reference-walkers** *(DONE — 16eb829, a9a1221)*:
     `GeneratedSource.autoFacets`, `ReflectionDomain.referencedViewables` iterate
     `FieldSet.of(q).fields()` instead of the raw map / a reflection fork.
   - **3c — the builder fork** *(NOT a mechanical refactor — see Findings)*:
     `ViewablePanel.renderFields`/`appendDynamicEntry` and `ViewableJson`'s main dispatch
     (`dynamicFields()` vs the declared loop) + `fieldOfSingle`. These fork because a
     declared field carries render annotations (`@Link`, `@ViewableInline`,
     `@ViewableReference`, `ImageRef`, `@Numeric`) that a dynamic map value cannot, and
     the two builders even encode *different* semantics (a declared Viewable field always
     emits a `ref`; a dynamic Viewable field emits `ref`-or-external-`link`). Unifying
...
- `QueryObjectResultPanel.collectReferences` — same walker shape as `ReflectionDomain`
  ...
  yet, so migrating would drop that skip for typed Viewables. Deferred until 3c adds the
...
from the Viewable contract, `@NotViewableField`/provenance skips, image-field filter).

## Out of scope

- `FieldTypeSource` / modeled type labels for references (the "ForWork/Viewable"
