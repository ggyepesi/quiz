...
public class ViewableSearchPanel extends JPanel
        implements ViewablePanelTargetListener {
    ...
    private ViewableRenderContext renderContext;
    ...
    private final ViewablePanelSearchAndSort searchAndSort =
            new ViewablePanelSearchAndSort();

    private final Set<JComponent> rememberedSearchComponents =
            Collections.newSetFromMap(new IdentityHashMap<>());

    private final Set<ViewablePanel> previousMatchedCards =
            Collections.newSetFromMap(new IdentityHashMap<>());
    ...
    public void setRenderContext(ViewableRenderContext context) {
     ...
     */
    @Override
    public void quizablePanelsAdded(List<ViewablePanel> added) {
        if (targetPanel == null) {
            return;
        }
     ...
     */
    @Override
    public void quizablePanelsUpdated(List<ViewablePanel> updated) {
        if (targetPanel == null) {
            return;
        }
    ...
    @Override
    public void quizablePanelMaterialized(ViewablePanel card) {
    ...
    private void sortTargetPanels() {
        ...
        if (virtualList != null) {
            ...
            Viewable anchor = currentHit instanceof ViewablePanel qp
                    ? qp.getQuizable()
    ...
    public ViewableSearchPanel(Class<? extends Viewable> cls) {
        this(cls, null);
    }
    ...
    public ViewableSearchPanel(Class<? extends Viewable> cls, Viewable sample) {
        this.searchClass = cls;
        setLayout(new BorderLayout(6, 6));

        ViewablePanelConfig nameOnly =
                nameOnlyConfig(cls);

        ViewablePanelConfig viewBase =
                ViewablePanelConfig.all(cls);

        searchEditor =
                new ViewablePanelConfigEditor(nameOnly.copy(), true, sample);

        sortEditor =
                new ViewablePanelConfigEditor(nameOnly.copy(), true, sample);

        viewEditor =
                new ViewablePanelConfigEditor(viewBase.copy(), false, sample);
    ...
    private ViewablePanelConfig nameOnlyConfig(
            Class<? extends Viewable> cls) {

        ViewablePanelConfig cfg =
                ViewablePanelConfig.of(cls);
    ...
    public ViewablePanelConfig getSearchConfig() {
        return searchEditor.getConfig();
    }

    public ViewablePanelConfig getSortConfig() {
        return sortEditor.getConfig();
    }

    public ViewablePanelConfig getViewConfig() {
    ...
    public ViewablePanelConfigEditor searchEditor() { return searchEditor; }
    public ViewablePanelConfigEditor sortEditor() { return sortEditor; }
    public ViewablePanelConfigEditor viewEditor() { return viewEditor; }
    ...
    private void searchSync(String query) {
        ...
        Map<String, List<ViewablePanel>> matchesByField =
                searchAndSort.search(queryTokens);

        Map<String, HitGroup> groups =
                new LinkedHashMap<>();

        for (Map.Entry<String, List<ViewablePanel>> e
                : matchesByField.entrySet()) {

            HitGroup group =
                    new HitGroup(e.getKey());

            for (ViewablePanel qp : e.getValue()) {
    ...
    private void addFieldHighlights(
        ...
        for (ViewablePanel qp : toRefresh) {
            qp.refresh();
        }
    ...
    private void highlightCard(ViewablePanel qp) {
        remember(qp);
    ...
    private void collectMatchingFieldRows(
        ...
        if (root instanceof ViewableTextBlock block) {
    ...
    private void applyViewConfig(boolean searchAfter) {
        ...
        for (Viewable q : quizables) {
            ...
            ViewablePanel panel =
                    new ViewablePanel(q, viewCfg, context, false);
    ...
    private void highlightLabelsUnder(
        ...
        if (root instanceof ViewableTextRow row) {
            remember(row);
    ...
    private void restoreRememberedComponent(JComponent jc) {
        if (jc instanceof ViewableTextRow row) {
            row.clearHighlight();
        }

        if (jc instanceof ViewableTextBlock block) {
        ...
        if (jc instanceof ViewablePanel qp) {
     ...
     * leaf card. The ordinary ViewablePanel expansion/highlight machinery is
    ...
    private void navigateToCurrentVirtual(HitGroupQ g) {
        ...
        if (card instanceof ViewablePanel qp) {
            if (g.fieldPath != null
