...
public class ViewablePanelView {
    ...
    private ViewableRenderContext context;

    // Column count and trailing glue filler, remembered so a live add can
    // place the next card.
    private int columns = 1;

    private final List<ViewablePanelTargetListener> targetListeners =
            new ArrayList<>();
    ...
    private ViewableRenderContext sharedContext;
    private boolean inPlaceNavigation;

    public ViewablePanelView() {
    }
     ...
     * must pre-register every object (via {@link ViewableRenderContext}) so
     ...
     */
    public void addTargetListener(ViewablePanelTargetListener listener) {
    ...
    public void createCardsPanel(int numColumns) {
        ...
        virtualList.setOnCardBuilt(card -> {
            if (card instanceof ViewablePanel qp) {
                for (ViewablePanelTargetListener listener : targetListeners) {
    ...
    private javax.swing.JComponent buildVirtualCard(Viewable q) {
        ViewablePanel panel = buildQuizableCard(q);
    ...
    private ViewablePanel buildQuizableCard(Viewable q) {
        ViewablePanelConfig cfg =
                ViewablePanelConfig.all(q.getClass())
                                   .setAddListener(true)
                                   .setThumb(true);
        ...
        ViewablePanel panel =
                new ViewablePanel(q, cfg, context, false);
    ...
    public void addQuizableLive(Viewable q) {
        ...
        ViewablePanel panel = virtualList.builtCard(q) instanceof ViewablePanel p
                ? p : null;
        for (ViewablePanelTargetListener listener : targetListeners) {
    ...
    private ViewableRenderContext resolveContext() {
        ViewableRenderContext c =
                sharedContext != null
                        ? sharedContext
    ...
    private ViewablePanel findCard(Viewable q) {
        return virtualList != null && virtualList.builtCard(q) instanceof ViewablePanel qp
                ? qp : null;
    }
    ...
    public ViewableRenderContext getRenderContext() {
        return context;
    }
    ...
    private void createCardsBad() {
        ...
        // Second pass: create direct ViewablePanel cards.
        for (Viewable q : quizables) {
            if (q == null) {
                continue;
            }
            ...
            if (cfg == null) {
                cfg = ViewablePanelConfig.all(q.getClass())
                                         .setAddListener(true)
                                         .setThumb(true);
            }
    ...
    private void tuneCardSize(JPanel panel) {
        ...
        if (panel instanceof ViewablePanel card) {
