...
public class ViewableRenderContext {

    private static final Logger log = LoggerFactory.getLogger(ViewableRenderContext.class);
    ...
    private final Map<Class<?>, ViewablePanelConfig> classConfigs =
            new HashMap<>();
    ...
    public ViewableRenderContext(Collection<? extends Viewable> quizables) {
        if (quizables != null) {
    ...
    public void putClassConfig(Class<?> cls, ViewablePanelConfig config) {
        if (cls != null && config != null) {
            classConfigs.put(cls, config.copy());
        }
    }

    public ViewablePanelConfig configFor(Class<?> cls) {
        if (cls == null) {
            return null;
        }

        ViewablePanelConfig exact = classConfigs.get(cls);

        if (exact != null) {
            return exact.copy();
        }

        for (Map.Entry<Class<?>, ViewablePanelConfig> e : classConfigs.entrySet()) {
