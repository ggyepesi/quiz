 ...
 *   <li>{@link ViewableReference @ViewableReference} — explicit chip;
 ...
 *       clickable link row (see {@link ViewableLinkRow}).</li>
 *   <li>{@code @NotViewableField} — not rendered. {@code @MinorField} —
 ...
 * {@link ViewableRenderContext} and rebuilds the card via {@link
 ...
 *
 * <h3>Components</h3>
 * Text is drawn by lightweight painted components ({@link ViewableTextRow},
 * {@link ViewableTextBlock}, {@link ViewableReferenceRow}, {@link
 * ViewableLinkRow}) rather than per-value Swing widgets, so a card with
 * tens of thousands of fields stays cheap. The only structural extra is a
 * single top-pinning {@link Box.Filler} per root card.
 */
public class ViewablePanel extends JPanel {

    private static final Logger log = LoggerFactory.getLogger(ViewablePanel.class);
    ...
    private final ViewablePanelConfig config;
    ...
    private final ViewableRenderContext renderContext;
    ...
    public ViewablePanel(Viewable quizable,
                         ViewablePanelConfig config,
                         boolean fill) {
        this(identitySetOf(), identitySetOf(), new ViewableRenderContext(),
    ...
    public ViewablePanel(Viewable quizable,
                         ...
                         boolean fill) {
        this(identitySetOf(), identitySetOf(), new ViewableRenderContext(topLevel),
                true, quizable, config, fill, new ArrayList<>(), null, null);
    }

    public ViewablePanel(Viewable quizable,
                         ViewablePanelConfig config,
                         ViewableRenderContext renderContext,
                         boolean fill) {
    ...
    public ViewablePanel(Set<Object> visited,
     ...
     * Arguments can't fit into locals in class file quiz/ui/ViewablePanel$RenderStats
     */
    ...
    public ViewablePanel(Set<Object> visited,
        ...
        this.renderContext = renderContext == null
                ? new ViewableRenderContext()
                : renderContext;
        this.fill = fill;
        this.path = path == null ? new ArrayList<>() : new ArrayList<>(path);

        this.config = config == null
                ? ViewablePanelConfig.of(quizable == null ? null : quizable.getClass())
                : config;
    ...
    private JComponent collapsibleRootHeader(boolean expanded) {
        ...
        titleLabel.putClientProperty(ViewableSearchPanel.FIELD_NAME_PROPERTY, "name");
        titleLabel.putClientProperty(ViewableSearchPanel.FIELD_VALUE_PROPERTY, title);
        titleLabel.putClientProperty(ViewableSearchPanel.FIELD_PATH_PROPERTY, List.of("name"));
    ...
    private void addCompactReference(Viewable q, boolean focusTopLevel) {
        ViewablePanelConfig openCfg = configForNested(q);

        addSingle(
                new ViewableReferenceRow(
                        "",
                        namePath(path),
                        q,
                        renderContext,
    ...
    private void buildFields() {
        ...
        List<ViewableTextBlock.Row> textRows = new ArrayList<>();
        ...
        for (Field field : config.visibleFieldsFor(quizable.getClass())) {
            ...
            if (value instanceof Boolean flag) {
                if (flag) {
                    textRows.add(new ViewableTextBlock.Row(
    ...
    private int addTextBlock(List<ViewableTextBlock.Row> rows, int row) {
        ViewableTextBlock block = new ViewableTextBlock(rows);
    ...
    private int appendDynamicEntry(String key, Object value,
                                   List<ViewableTextBlock.Row> textRows, int row) {
        ...
        if (value instanceof Boolean flag) {
            if (flag) {
                textRows.add(new ViewableTextBlock.Row(null, fieldPath, value,
        ...
        ViewablePanelConfig cfg = config.getFieldConfig(key);
        if (cfg == null) {
            cfg = defaultConfigForValue(value);
        }
        JComponent comp = ViewableValueRenderer.createFieldComponent(
    ...
    private int addRenderedField(Field field, Object value, int row) {
        ...
        // @Provenance (a Source) renders like @ViewableReference: a collapsed
        ...
        // @ViewableInline means "always render fully expanded inline" (e.g. a
        ...
        if (ViewableAdapter.isLinkField(field)
            ...
            addSingle(new ViewableLinkRow(fieldName, fieldPath, url, label), row++);
            return row;
        }
        ...
        ViewablePanelConfig fieldCfg = config.getFieldConfig(fieldName);
        ...
        if (isCollectionOrMap) {
            ViewablePanelConfig cfg = fieldCfg;
            Object collValue = value;
            return addCollapsibleCollection(fieldName, fieldPath, value, row,
                    () -> ViewableValueRenderer.createFieldComponent(
                            copyVisited(), copyAncestors(), renderContext,
    ...
    private int addCollapsibleCollection(
        ...
        ViewableCollectionHeader header = new ViewableCollectionHeader(
    ...
    // Only reached for @ViewableInline fields, so the broad/cyclic graphs
    ...
    // it (see ViewableReferenceRow) flips renderContext expand state and
    ...
    private JComponent collapsibleReference(
        ...
        if (renderContext != null && renderContext.isTopLevel(target)) {
            return new ViewableReferenceRow(
                    fieldName,
                    namePath(fieldPath),
                    target,
                    renderContext,
    ...
    private ViewableTextBlock.Row textBlockRow(
            String fieldName,
            List<String> fieldPath,
            Object value) {
        ...
        return new ViewableTextBlock.Row(
                fieldName,
    ...
    private ViewablePanelConfig defaultConfigForValue(Object value) {
        if (value instanceof Viewable q) {
    ...
    private ViewablePanelConfig configForNested(Viewable q) {
        ViewablePanelConfig fromContext =
    ...
    private void openInFrame(Viewable q) {
        new ViewableFrame(q,
                          ViewablePanelConfig.allWithMinorFields(q.getClass())
                                             .setAddListener(config.isAddListener())
                                             .setThumb(config.isThumb()));
    }
