 ...
 * state in {@link ViewableRenderContext} (keyed by the collection's identity)
 * and rebuilds the card, mirroring {@link ViewableReferenceRow}: a collapsed
 * collection renders only this header, an expanded one renders the header plus
 * its items below.
 */
public class ViewableCollectionHeader extends JComponent {
    ...
    public ViewableCollectionHeader(String fieldName,
        ...
        putClientProperty(ViewableSearchPanel.FIELD_PATH_PROPERTY, path);
