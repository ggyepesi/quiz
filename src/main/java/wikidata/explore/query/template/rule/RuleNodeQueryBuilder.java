...
public final class RuleNodeQueryBuilder {
    ...
    /**
     * LIMIT must apply before sorting: a flat ORDER BY ?valueLabel over a
     * huge class (e.g. Q523 star, ~3M instances) materializes and sorts
     * every instance before the LIMIT and times out. Nesting lets the
     * inner query stream until its LIMIT, then only the returned rows
     * are sorted. The sampled rows are index-order, not alphabetical.
     */
