...
public record CompiledStatementSource(
    ...
    public boolean configured() {
        return !sourceClassName.isBlank() && propertyPid.matches("(?i)P\\d+");
    }

    public static CompiledStatementSource from(
            StatementClassSource source, String resolvedClassName) {
        if (source == null) return null;
        return new CompiledStatementSource(
                source.sourceClassName(), resolvedClassName, source.propertyPid());
    }
