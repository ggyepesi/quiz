 ...
 * state in {@link QuizableRenderContext} (keyed by the collection's identity)
 * and rebuilds the card, mirroring {@link QuizableReferenceRow}: a collapsed
 * collection renders only this header, an expanded one renders the header plus
 * its items below.
 */
public class QuizableCollectionHeader extends JComponent {
    ...
    public QuizableCollectionHeader(String fieldName,
        ...
        putClientProperty(QuizableSearchPanel.FIELD_PATH_PROPERTY, path);
