...
public class QuizableFilterConfig {
    private final List<QuizableFieldOperation> operations =
            new ArrayList<>();

    private final Map<Class<?>, Map<String, Field>> fieldCache =
            new ConcurrentHashMap<>();

    public void addOperation(QuizableFieldOperation op) {
        if (op != null) {
            operations.add(op);
        }
    }

    public List<QuizableFieldOperation> getOperations() {
        return operations;
    }
    ...
    public boolean matches(Quizable q) {
        if (q == null) {
            return false;
        }

        for (QuizableFieldOperation op : operations) {
    ...
    private boolean matchesOperation(
            Object value,
            QuizableFieldOperation op
    ) {
        return switch (op.getKind()) {
            case EXISTS -> QuizableAdapter.isValidQuizValue(value);
            case EMPTY -> !QuizableAdapter.isValidQuizValue(value);
