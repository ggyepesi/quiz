...
class DynamicSearchConfigTest {
    ...
    private static QuizablePanelConfig explicit(String... fieldNames) {
        QuizablePanelConfig cfg = QuizablePanelConfig.of(DynamicQuizable.class);
        cfg.setAllFields(false);
        for (String f : fieldNames) {
            cfg.addField(f, QuizablePanelConfig.leaf());
        }
        return cfg;
    }
    ...
    @Test void configNamedDynamicFieldYieldsAPath() {
        List<QuizableFieldPaths.FieldPath> paths =
                QuizableFieldPaths.collect(explicit("won"));
